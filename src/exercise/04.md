...
The message of when to use `useLayoutEffect` and `useEffect` has been heard loud and clear: if I need to perform changes on the DOM, better to do it using a `useLayoutEffect` to make those changes before the browser repaints the UI. If not, better use `useEffect` so as to not slow down the repaint of the UI.

But playing with the example did not exactly help (I guess because of the change in React 18). The linked blog post and the video recorded before Node 18 definitely did.
## Background
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=Advanced%20React%20Hooks%20%F0%9F%94%A5&e=04%3A%20useLayoutEffect%3A%20auto-scrolling%20textarea&em=jaime.ferbec%40gmail.com).
